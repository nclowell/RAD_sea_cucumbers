...
parser.add_argument("-d", "--dir", help="path to ipyrad directory, without final slash", type=str, required = True)
...
# iterate through assembles, extract information, and store in lists
...
# proportion homozygous loci from retained_loci
...
props_het = []
for i in [x/y for x, y in zip(pi_0, ret_loci_after_minsample)]:
    props_het.append(1-i)
plt.plot(clusts,props_het)
plt.title("Proportion of heterozygous loci")
plt.xlabel("Clustering similarity %")
plt.ylabel("1 - proportion homozygous, after filter for max alleles = 2")
plt.savefig("clust_eval_plots/prop_het.png")
plt.close()

# proportion filtered by max alleles = 2
...
# proportion loci lost to remove duplicates
